...
ALLOW_ERROR_INJECTION(kmem_cache_alloc, NONE);



void *kmem_cache_alloc_for_bpf(struct kmem_cache *s, gfp_t gfpflags)
{
	void *ret = slab_alloc(s, gfpflags, _RET_IP_, s->object_size);
...
EXPORT_SYMBOL(kmem_cache_alloc_for_bpf);
...
ALLOW_ERROR_INJECTION(kmem_cache_alloc_node, NONE);
...
ALLOW_ERROR_INJECTION(kmem_cache_alloc_node_trace, NONE);
#endif
#endif	/* CONFIG_NUMA */
...
{
	if (x > 0xffff900000000000)
		pr_info("kmem_cache_free:%016lx\n", x);
	if (is_vmalloc_addr(x)) {
		vfree(x);
		pr_info("it's a vmalloc addr\n");
		return ;
	}
	kfree(x);
	return;
...
ALLOW_ERROR_INJECTION(kmem_cache_free, NONE);
...
ALLOW_ERROR_INJECTION(kmem_cache_free_bulk, NONE);
...
ALLOW_ERROR_INJECTION(kmem_cache_alloc_bulk, NONE);
...
ALLOW_ERROR_INJECTION(__kmalloc_node, NONE);
#endif	/* CONFIG_NUMA */
...
void kfree_for_bpf(const void *x)
{
	struct page *page;
	void *object = (void *)x;
...
ALLOW_ERROR_INJECTION(__kmalloc_track_caller, NONE);
...
ALLOW_ERROR_INJECTION(__kmalloc_node_track_caller, NONE);
#endif
