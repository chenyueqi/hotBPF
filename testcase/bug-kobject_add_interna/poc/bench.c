                                  ...
                                  size_t data_len)
{
  ...
  // XXX invoke hci_conn_add net/bluetooth/hci_conn.c:525
  uint64_t start = current_time_ns();
  writev(fd, iv, sizeof(iv) / sizeof(struct iovec));
  fprintf(stderr, "[hci_conn_add]:%lu\n", current_time_ns()-start);
  /*
...
static void initialize_vhci()
{
  ...
  // XXX invoke hci_alloc_dev net/bluetooth/hci_core.c:3376
  uint64_t start = current_time_ns();
  read(vhci_fd, &vendor_pkt, sizeof(vendor_pkt)) != sizeof(vendor_pkt);
  fprintf(stderr, "[hci_alloc_dev]:%lu\n", current_time_ns()-start);
  /*
...
static int do_sandbox_none(void)
{
  ...
  fprintf(stderr, "[bench] initialize_vhci\n");
  initialize_vhci();
  fprintf(stderr, "[bench] loop\n");
  loop();
  exit(1);
}
...
{
      setup_test();
      fprintf(stderr, "[bench] execute_one\n");
      execute_one();
      close_fds();
      exit(0);
      /*
...
int main(void)
{
  ...
  fprintf(stderr, "[bench] do_sandbox_none\n");
  do_sandbox_none();
  while(1);
  return 0;
}
